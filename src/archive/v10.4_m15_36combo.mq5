//+------------------------------------------------------------------+
//|                    PairAB_BasketScaleIn_v10.4.mq5                |
//|                    配对交易 - Basket阶梯加仓版                   |
//|                    Z方向统一 + 回归后盈利平仓                    |
...
#property description "本轮固定OLS residual + M15，仅优化首仓Z/加仓Z间距/线性手数增量，共36组合，并输出综合Score"
#property strict
...
input double  InpZScoreOpen = 3.0;             // 首仓阈值：本轮优化3.0/3.5/4.0/4.5
input double  InpZScoreClose = 0.0;            // 回归阈值：正Basket Z<=该值；负Basket Z>=-该值

//=== 阶梯加仓参数 ===
input bool    InpEnableScaleIn = true;          // 是否启用连续偏离加仓
input double  InpAddZStep = 1.5;             // 加仓Z间距：本轮优化1.0/1.5/2.0
input int     InpMaxAddCount = 7;               // 最多加仓次数（不含首仓）
input double  InpAddLotStep = 0.03;             // 线性加仓增量：本轮优化0.02/0.03/0.04
...
input bool    InpExportOptimizationCSV = true; // 兼容参数；v10.4完成优化时强制导出CSV
...
int OnInit()
{
    ...
    SpreadModel = SPREAD_OLS_LOG_RESIDUAL; // v10.4寻优阶段固定OLS residual
    ...
    Print("=== 配对交易Basket阶梯加仓版 v10.4 OLS M15寻优 ===");
...
int OnTesterInit()
{
    // 本轮研究目标固定：OLS residual + M15，寻找三个参数的最佳组合。
    ...
    // 三个核心寻优参数：4 × 3 × 3 = 36 个完整组合。
    // ZScoreOpen: 3.0 / 3.5 / 4.0 / 4.5
    // AddZStep:   1.0 / 1.5 / 2.0
    // AddLotStep: 0.02 / 0.03 / 0.04
    ok = ParameterSetRange("InpZScoreOpen", true, 3.0, 3.00, 0.50, 4.50) && ok;
    ok = ParameterSetRange("InpAddZStep", true, 1.5, 1.00, 0.50, 2.00) && ok;
    ok = ParameterSetRange("InpAddLotStep", true, 0.03, 0.02, 0.01, 0.04) && ok;
    ...
    Print("✅ v10.4优化范围已锁定：OLS | M15 | ZOpen 3.0-4.5 step0.5 | AddZStep 1.0-2.0 step0.5 | AddLotStep 0.02-0.04 step0.01 | 36组合");
    return INIT_SUCCEEDED;
}
...
void OnTesterDeinit()
{
    // v10.4寻优版强制导出结果表，避免旧.set把该开关设为false后丢失评估表。
