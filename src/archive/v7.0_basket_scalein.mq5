...
#property description "支持极端偏离阶梯加仓、Basket回归后盈利平仓、冷却期和状态恢复"
#property strict
...
input double  InpAddZStep = 0.50;               // 每继续偏离多少Z加仓一次
input int     InpMaxAddCount = 3;               // 最多加仓次数（不含首仓）
input double  InpAddLotMultiplier = 1.00;       // 每层手数倍率：LevelN = 基础手数 * 倍率^N
input int     InpMinAddIntervalSec = 60;        // 两次加仓最短间隔（秒）
input double  InpMaxAbsZ = 5.0;                 // |Z|达到/超过此值后禁止继续加仓（<=0禁用）
...
//=== 资金与风险管理 ===
input double  InpLots = 0.1;                   // 基准手数（品种1/单腿模式基础手数）
...
int OnInit()
{
    ...
    AddLotMultiplier = InpAddLotMultiplier;
    ...
    if(LookBack_Period < 2 || ZScore_Open <= 0 || Lots <= 0 ||
       AddZStep <= 0 || MaxAddCount < 0 || AddLotMultiplier <= 0 ||
    ...
    Print("阶梯加仓: ", EnableScaleIn ? "启用" : "禁用",
          ...
          " | 最多加仓=", MaxAddCount,
          " | 手数倍率=", DoubleToString(AddLotMultiplier, 2));
...
double GetLevelBaseLots(int level)
{
    if(level < 0)
        return 0.0;
    return Lots * MathPow(AddLotMultiplier, level);
}
...
void DisplayInfo()
{
    ...
    info += " Step=" + DoubleToString(AddZStep, 2) + " Max=" + IntegerToString(MaxAddCount);
    info += "\n盈利平仓: ";
