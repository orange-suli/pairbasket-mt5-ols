//+------------------------------------------------------------------+
//|                    PairAB_BasketScaleIn_v10.5.mq5                |
//|                    配对交易 - Basket阶梯加仓版                   |
//|                    Z方向统一 + 回归后盈利平仓                    |
...
#property description "本轮固定OLS residual + M15，三套策略预设 × MaxAddCount(7/9/11) × ZClose(0/0.5/1)，共27组合，并输出综合Score"
#property strict
...
//=== 三套固定策略预设（本轮只优化预设编号，不再拆开混搭OpenZ/AddZ/LotStep） ===
enum ENUM_STRATEGY_PRESET
{
   PRESET_HIGH_RETURN = 0,    // A 高收益型：OpenZ=3.0, AddZ=1.0, LotStep=0.04
   PRESET_BALANCED = 1,       // B 平衡型：OpenZ=4.5, AddZ=2.0, LotStep=0.03
   PRESET_HIGH_QUALITY = 2    // C 高质量稍激进：OpenZ=4.5, AddZ=2.0, LotStep=0.04
};
input ENUM_STRATEGY_PRESET InpStrategyPreset = PRESET_BALANCED; // 本轮优化A/B/C三套固定版本

//=== 核心策略参数 ===
input int     InpLookBack = 60;                // Z-Score回看周期
input double  InpZScoreClose = 0.0;            // 提前回归阈值：本轮优化0/0.5/1.0

//=== 阶梯加仓参数 ===
input bool    InpEnableScaleIn = true;          // 是否启用连续偏离加仓
input int     InpMaxAddCount = 7;               // 最多加仓次数（不含首仓）：本轮优化7/9/11
input int     InpMinAddIntervalSec = 60;        // 两次加仓最短间隔（秒）
input double  InpMaxAbsZ = 30.0;                // 本轮固定30，避免旧的10把7/9/11加仓次数提前截断
...
input bool    InpExportOptimizationCSV = true; // 兼容参数；v10.5完成优化时强制导出CSV
input string  InpOptimizationCsvName = "PairBasket_v10_4_OLS_M15_OptimizationMetrics.csv"; // 输出到Common\\Files
...
#define OPT_FRAME_NAME "PAIR_OPT_METRICS_V10_4"
...
int OnInit()
{
    ...
    Print("=== 配对交易Basket阶梯加仓版 v10.5 OLS M15 27组合验证 ===");
...
{
    if(preset == PRESET_HIGH_RETURN) return "A_HIGH_RETURN_3.0_1.0_0.04";
    if(preset == PRESET_BALANCED) return "B_BALANCED_4.5_2.0_0.03";
    if(preset == PRESET_HIGH_QUALITY) return "C_HIGH_QUALITY_4.5_2.0_0.04";
    return "UNKNOWN";
}

void GetPresetParameters(int preset, double &openZ, double &addZ, double &lotStep)
{
    if(preset == PRESET_HIGH_RETURN)
    {
        openZ = 3.0; addZ = 1.0; lotStep = 0.04;
        return;
    }
    if(preset == PRESET_BALANCED)
    {
        openZ = 4.5; addZ = 2.0; lotStep = 0.03;
        return;
    }
    // PRESET_HIGH_QUALITY 以及异常值都按C输出；异常值不会通过OnInit校验。
    openZ = 4.5; addZ = 2.0; lotStep = 0.04;
}
...
int OnTesterInit()
{
    // v10.5：固定OLS + M15。只优化三项：策略预设 × 最大加仓次数 × 提前回归阈值。
    // 3 × 3 × 3 = 27 个完整组合。
    bool ok = true;
    ...
    // 最大加仓次数：7 / 9 / 11
    ok = ParameterSetRange("InpMaxAddCount", true, (long)7, (long)7, 2, (long)11) && ok;

    // 提前回归平仓：0 / 0.5 / 1.0
    ok = ParameterSetRange("InpZScoreClose", true, 0.0, 0.0, 0.5, 1.0) && ok;

    // 其余参数全部固定。MaxAbsZ提高到30，避免旧值10提前截断7/9/11的测试。
    ...
    Print("✅ v10.5优化范围已锁定：OLS | M15 | 3策略预设 × MaxAddCount(7/9/11) × ZClose(0/0.5/1.0) = 27组合 | MaxAbsZ=30");
    return INIT_SUCCEEDED;
}
...
void OnTesterDeinit()
{
    // v10.5验证版强制导出结果表，避免旧.set把该开关设为false后丢失评估表。
    ...
    while(FrameNext(pass, frameName, frameId, customValue, metrics))
    {
        ...
        GetPresetParameters(presetValue, presetOpenZ, presetAddZ, presetLotStep);
