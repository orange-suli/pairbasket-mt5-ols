//+------------------------------------------------------------------+
//|                    PairAB_BasketScaleIn_v10.0.mq5                |
//|                    配对交易 - Basket阶梯加仓版                   |
//|                    Z方向统一 + 回归后盈利平仓                    |
...
#property description "支持双Spread模型、线性阶梯加仓、优化风险指标导出、Basket回归后盈利平仓"
#property strict
...
input ENUM_SPREAD_MODEL InpSpreadModel = SPREAD_SMA_RELATIVE;
...
input bool    InpExportOptimizationCSV = true; // 完成优化后导出每个Pass的自定义风险指标
input string  InpOptimizationCsvName = "PairBasket_v10_OptimizationMetrics.csv"; // 输出到Common\\Files
...
#define OPT_FRAME_NAME "PAIR_OPT_METRICS_V10"
#define OPT_FRAME_ID   10001
...
int OnInit()
{
    ...
    SpreadModel = (int)InpSpreadModel;
...
//+------------------------------------------------------------------+
//| 单Pass结束：把自定义指标发送给优化主终端，并返回Sharpe作为Custom max |
//+------------------------------------------------------------------+
double OnTester()
{
    UpdateTesterRiskMetrics();
    ...
    // 只在优化Pass中发送Frame。单次测试仍可在报告里看到OnTester返回的Sharpe。
    if((bool)MQLInfoInteger(MQL_OPTIMIZATION))
    {
        if(!FrameAdd(OPT_FRAME_NAME, OPT_FRAME_ID, sharpe, metrics))
            Print("⚠️ FrameAdd失败，错误=", GetLastError());
    }

    // 在Strategy Tester中选择“Custom max”时，可直接按Sharpe排序。
    return sharpe;
}
...
void OnTesterDeinit()
{
    ...
    while(FrameNext(pass, frameName, frameId, customValue, metrics))
    {
        ...
        FileWrite(handle,
                  (long)pass,
                  FrameSpreadModelName(spreadRaw),
