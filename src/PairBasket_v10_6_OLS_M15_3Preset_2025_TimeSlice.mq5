//+------------------------------------------------------------------+
//|                    PairAB_BasketScaleIn_v10.6.mq5                |
//|                    配对交易 - Basket阶梯加仓版                   |
//|                    Z方向统一 + 回归后盈利平仓                    |
...
#property description "2025时间切片验证：固定OLS residual + M15，仅比较三套已选策略预设，共3组合，并输出综合Score"
#property strict
...
//=== 分析周期：保留M15/M30枚举；本轮OnTesterInit固定M15，不优化周期 ===
enum ENUM_OPT_TIMEFRAME
{
   OPT_TF_M15 = 0,   // M15
   OPT_TF_M30 = 1    // M30
};
input ENUM_OPT_TIMEFRAME InpTimeFrame = OPT_TF_M15; // 本轮优化固定M15
...
//=== 三套固定策略预设（时间切片只枚举预设，不再拆开混搭参数） ===
enum ENUM_STRATEGY_PRESET
{
   PRESET_HIGH_RETURN = 0,    // A：OpenZ=3.0, CloseZ=0.0, AddZ=1.0, MaxAdd=7, LotStep=0.04
   PRESET_BALANCED = 1,       // B：OpenZ=4.5, CloseZ=1.0, AddZ=2.0, MaxAdd=7, LotStep=0.03
   PRESET_HIGH_QUALITY = 2    // C：OpenZ=4.5, CloseZ=1.0, AddZ=2.0, MaxAdd=7, LotStep=0.04
};
input ENUM_STRATEGY_PRESET InpStrategyPreset = PRESET_BALANCED; // 2025时间切片只优化A/B/C三个版本

//=== 核心策略参数 ===
input int     InpLookBack = 60;                // Z-Score回看周期
input double  InpZScoreClose = 0.0;            // 兼容输入；v10.6由策略预设覆盖（A=0，B/C=1）

//=== 阶梯加仓参数 ===
input bool    InpEnableScaleIn = true;          // 是否启用连续偏离加仓
input int     InpMaxAddCount = 7;               // 兼容输入；v10.6三个预设均固定MaxAdd=7
input int     InpMinAddIntervalSec = 60;        // 两次加仓最短间隔（秒）
input double  InpMaxAbsZ = 30.0;                // 固定30，与v10.5验证口径保持一致
...
//=== 优化统计 ===
input double  InpExtremeZThreshold = 3.0;      // 极端Z阈值；<=0时自动使用当前预设的OpenZ
input bool    InpExportOptimizationCSV = true; // 兼容参数；v10.6完成优化时强制导出CSV
input string  InpOptimizationCsvName = "PairBasket_v10_6_OLS_M15_3Preset_2025_OptimizationMetrics.csv"; // 输出到Common\\Files
...
#define OPT_FRAME_NAME "PAIR_OPT_METRICS_V10_6"
#define OPT_FRAME_ID   10013
...
int OnInit()
{
    ...
    SpreadModel = SPREAD_OLS_LOG_RESIDUAL; // v10.6时间切片阶段固定OLS residual
    StrategyPreset = (int)InpStrategyPreset;
    RegressionLookBack = InpRegressionLookBack;
    LookBack_Period = InpLookBack;

    // 三套策略参数必须作为一个整体切换。
    // 时间切片阶段同时固定 OpenZ / CloseZ / AddZ / MaxAdd / LotStep，避免任何参数混搭。
    switch(InpStrategyPreset)
    {
        case PRESET_HIGH_RETURN:
            ZScore_Open = 3.0;
            ZScore_Close = 0.0;
            AddZStep = 1.0;
            MaxAddCount = 7;
        ...
        case PRESET_HIGH_QUALITY:
            ...
            break;
        default:
            Print("❌ 未知策略预设: ", (int)InpStrategyPreset);
    ...
    // MaxAddCount 已由策略预设固定为7；InpMaxAddCount仅保留兼容旧.set文件。
    ...
    {
        Print("❌ InpMaxAbsZ不能小于当前策略预设的OpenZ；如不使用MaxAbsZ请设置<=0");
        return INIT_PARAMETERS_INCORRECT;
    }
    ...
    Print("=== 配对交易Basket阶梯加仓版 v10.6 OLS M15 三预设时间切片验证 ===");
    Print("Spread模型: ", SpreadModelToString(SpreadModel));
    Print("策略预设: ", StrategyPresetName(StrategyPreset));
...
int FrameStrategyPresetValue(string rawValue)
{
    if(StringFind(rawValue, "HIGH_RETURN") >= 0) return PRESET_HIGH_RETURN;
    if(StringFind(rawValue, "BALANCED") >= 0) return PRESET_BALANCED;
    if(StringFind(rawValue, "HIGH_QUALITY") >= 0) return PRESET_HIGH_QUALITY;
    return (int)StringToInteger(rawValue);
}

string StrategyPresetName(int preset)
{
    if(preset == PRESET_HIGH_RETURN) return "A_HIGH_RETURN_3.0_1.0_0.04_CLOSE0_MAXADD7";
    if(preset == PRESET_BALANCED) return "B_BALANCED_4.5_2.0_0.03_CLOSE1_MAXADD7";
    if(preset == PRESET_HIGH_QUALITY) return "C_HIGH_QUALITY_4.5_2.0_0.04_CLOSE1_MAXADD7";
    return "UNKNOWN";
}

void GetPresetParameters(int preset,
                         double &openZ,
                         double &closeZ,
                         double &addZ,
                         int &maxAdd,
                         double &lotStep)
{
    ...
    if(preset == PRESET_HIGH_RETURN)
    {
        openZ = 3.0; closeZ = 0.0; addZ = 1.0; lotStep = 0.04;
        return;
    }
    if(preset == PRESET_BALANCED)
    {
        openZ = 4.5; closeZ = 1.0; addZ = 2.0; lotStep = 0.03;
        return;
    }
    // PRESET_HIGH_QUALITY 以及异常值都按C输出；异常值不会通过OnInit校验。
    openZ = 4.5; closeZ = 1.0; addZ = 2.0; lotStep = 0.04;
}
...
int OnTesterInit()
{
    // v10.6时间切片：固定 OLS + M15，只比较三套最终候选。
    // 每套预设内部已经固定 CloseZ 与 MaxAdd，因此本轮只有 3 个完整Pass。
    bool ok = true;

    ok = ParameterSetRange("InpSpreadModel", false,
                            (long)SPREAD_OLS_LOG_RESIDUAL, 0, 0, 0) && ok;
    ok = ParameterSetRange("InpTimeFrame", false,
                            (long)OPT_TF_M15, 0, 0, 0) && ok;

    // A: 3.0 / Close0 / AddZ1.0 / MaxAdd7 / LotStep0.04
    // B: 4.5 / Close1 / AddZ2.0 / MaxAdd7 / LotStep0.03
    // C: 4.5 / Close1 / AddZ2.0 / MaxAdd7 / LotStep0.04
    ok = ParameterSetRange("InpStrategyPreset", true,
                            (long)PRESET_BALANCED,
                            (long)PRESET_HIGH_RETURN, 1, (long)PRESET_HIGH_QUALITY) && ok;

    // 以下两个旧输入仅为兼容参数；实际值在OnInit中由策略预设覆盖。
    ok = ParameterSetRange("InpMaxAddCount", false, (long)7, 0, 0, 0) && ok;
    ok = ParameterSetRange("InpZScoreClose", false, 0.0, 0.0, 0.0, 0.0) && ok;
    ...
    ok = ParameterSetRange("InpMaxAbsZ", false, 30.0, 0.0, 0.0, 0.0) && ok;
    ...
    ok = ParameterSetRange("InpDebugPrint", false, (long)false, 0, 0, 0) && ok;

    if(!ok)
    {
        Print("❌ OnTesterInit设置时间切片参数失败，error=", GetLastError());
        return INIT_FAILED;
    }

    Print("✅ v10.6时间切片已锁定：OLS | M15 | A/B/C三套最终候选 = 3组合 | MaxAdd=7 | A Close0，B/C Close1 | MaxAbsZ=30");
    return INIT_SUCCEEDED;
}
...
void OnTesterDeinit()
{
    // v10.6时间切片版强制导出结果表，避免旧.set把该开关设为false后丢失评估表。
    ...
    while(FrameNext(pass, frameName, frameId, customValue, metrics))
    {
        ...
        string presetRaw = FindFrameInputValue(parameters, parametersCount, "InpStrategyPreset");
        string spreadName = FrameSpreadModelName(spreadRaw);
        if(spreadName == "" || spreadName == spreadRaw)
            spreadName = "OLS_LOG_RESIDUAL";

        int presetValue = FrameStrategyPresetValue(presetRaw);
        double presetOpenZ = 0.0;
        double presetCloseZ = 0.0;
        double presetAddZ = 0.0;
        int presetMaxAdd = 7;
        double presetLotStep = 0.0;
        GetPresetParameters(presetValue,
                            presetOpenZ,
                            presetCloseZ,
                            presetAddZ,
                            presetMaxAdd,
        ...
        FileWrite(handle,
                  ...
                  StrategyPresetName(presetValue),
                  ...
                  DoubleToString(presetOpenZ, 2),
                  DoubleToString(presetCloseZ, 2),
                  DoubleToString(presetAddZ, 2),
                  presetMaxAdd,
                  DoubleToString(presetLotStep, 2),
...
void DisplayInfo()
{
    ...
    string info = "\n=== Z-Score Basket阶梯配对 v10.6 ===";
    info += "\n周期: " + timeFrameStr + " Z回看: " + IntegerToString(LookBack_Period);
    info += "\n预设: " + StrategyPresetName(StrategyPreset);
    if(SpreadModel == SPREAD_SMA_RELATIVE)
